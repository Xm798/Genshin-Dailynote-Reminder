...
3. Configure the sending mode `CQHTTP_MESSAGE_TYPE`, `private` for private chat sending, `group` for group chat sending, `guild` for QQ guild sending. You should get channel id and subchannel id before sending to QQ guild, see CQHTTP document [频道 API](https://docs.go-cqhttp.org/api/guild.html).
