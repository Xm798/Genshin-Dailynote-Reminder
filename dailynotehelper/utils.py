...
def tz_diff(tz_server: str, tz_local: str):
    offset_local = datetime.datetime.utcnow().replace(tzinfo=pytz.utc).astimezone(
        pytz.timezone(tz_local)).utcoffset().total_seconds() / 60 / 60
    offset_server = datetime.datetime.utcnow().replace(tzinfo=pytz.utc).astimezone(
        pytz.timezone(tz_server)).utcoffset().total_seconds() / 60 / 60
...
def reset_time_offset(region: str):
    tz_server = {'cn_gf01': 'Etc/GMT-8', 'cn_qd01': 'Etc/GMT-8',
                 'os_usa': 'Etc/GMT+5', 'os_euro': 'Etc/GMT-1', 'os_asia': 'Etc/GMT-8'}
