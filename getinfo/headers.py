...
class Headers:
    ...
    @staticmethod
    def set_cookie_exceed(cookie: str) -> None:
        ltuid = {item.split('=')[0]: item.split('=')[1] for item in cookie.split('; ')}.get('ltuid', -1)
        sql = 'update cookie set ltuid=%s, mark=day(now()) where cookie=%s'

    def create_dynamic_secret(self, query: dict, body: str) -> str:
        parameters: List[str] = [f'{k}={query[k]}' for k in sorted(query.keys())]
        q = '&'.join(parameters)
        ...
        check: str = self.md5(f"salt={salt}&t={time_}&r={random_}&b={body}&q={q}")
