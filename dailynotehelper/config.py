...
class Config:
    def __init__(self):
        ...
        try:
            ...
            print(
                'Cannot read the configuration file file! Please check if ./dailynotehelper/config/config.yaml exists.'
            )
            exit()
        else:
            ...
            for key in self.config_dict[k]:
                if 'COOKIE' in key or 'EXCLUDE_UID' in key:
                    self.config_data[key] = (
                        self.config_dict[k][key] if self.config_dict[k][key] else []
                    )
                else:
